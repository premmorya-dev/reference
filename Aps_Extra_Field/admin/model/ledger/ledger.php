...
class ModelLedgerLedger extends Model
{
    ...
    public function getAllPayment($data)
    {
        ...
        $sql = "SELECT * FROM " . DB_PREFIX . "apsinno_order_transaction_ledger";


        $implode = [];
        $implode[] = " transaction_type='credit'";

        if (!empty($data['filter_transaction_id_from']) && $data['filter_transaction_id_from'] != 'undefined') {
            $implode[] = " transaction_id >= '" . $this->db->escape($data['filter_transaction_id_from']) . "'";
        }

        if (!empty($data['filter_transaction_id_to']) && $data['filter_transaction_id_to'] != 'undefined') {
            $implode[] = " transaction_id <= '" . $this->db->escape($data['filter_transaction_id_to']) . "'";
        }


        if (!empty($data['filter_amount']) && $data['filter_amount'] != 'undefined') {
            $implode[] = " amount ='" . $this->db->escape($data['filter_amount']) . "'";
        }
    ...
    public function getLedgerCreditTransaction($id)
    {
        ...
        $data = [];
        $payment_html ='';
        // $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "apsinno_order_transaction_ledger where transaction_id = '" . $id . "' AND transaction_type = 'credit' ");
        $query = $this->db->query("Select * from  " . DB_PREFIX . "apsinno_order_transaction_ledger tl left join " . DB_PREFIX . "apsinno_order_payment_processor opp on tl.transaction_id=opp.transaction_id where tl.transaction_id ='" . $id . "' and tl.transaction_type ='credit' ")->row;

     
        if (isset($query['processor_type']) && $query['processor_type']) {

          
                $processor_name = $query['processor_type'] ;
           

            $payment = $this->db->query("Select * from " . DB_PREFIX . "apsinno_" .  $processor_name  . "_transactions where transaction_log_id='" . $query['transaction_log_id'] . "'")->row;
// print_r();die;
          
            if(!isset($payment['email'] )){
                $payment['email'] ='';
            }
            $customer = $this->db->query("Select CONCAT(firstname, ' ', lastname ) as customer_name  from  " . DB_PREFIX . "customer  where email ='" . $payment['email'] . "'")->row;
 
            if (isset($payment) && $payment && $processor_name == 'razorpay') {
            //  print_r($payment); die; 
            if($payment['captured'] == '1'){
                $payment['captured'] = '<span style="margin-left: 5px;"  class="label label-success">Captured</span>'; 
            }else{
                $payment['captured'] = '<span style="margin-left: 5px;" class="label label-danger">Failed</span>';    
            }
            // $payment['captured'] = $payment['captured'] ? 
                $payment_detail = [
                    ...
                    "payment_method_name" =>  $processor_name,
                ];

              $payment_html =  "<div class=\"col-md-6\">
                    <br /><label>Transaction Id</label> : ". $query['transaction_id'] ."
                    <br /><label>Payment Name</label> : ". $processor_name."
                    <br /><label>Method</label> : ". $payment['method'] ."
                    <br /><label>Razorpay Id</label> : ". $payment['razarpay_payment_id'] ."
                    <br /><label>Razorpay Order Id</label> : ". $payment['razorpay_order_id'] ."
                    <br /><label>Amount</label> : ". $payment['amount'] ."
                    <br /><label>Date</label> : ".$this->db->escape(date('d-M-Y h:i:A',$payment['created_at']))  ."
                   
                    <br /> <label>Currency</label> : ".$payment['currency']."
                 </div>
                <div class=\"col-md-6\">
                    <br /><label>Customer Name</label> : ".$customer['customer_name'] ."
                    <br /><label>Email</label> : ". $payment['email']."
                    <br /><label>Status</label> : ". $payment['captured']."
                </div>";

            }

            if (isset($payment) && $payment && $processor_name == 'bank') {
                // print_r($payment); die; 
                    $payment_detail = [
                        "customer_id" => $payment['customer_id'],
                        "method" => $payment['method'],
                  ...
                  $payment_html =  "<div class=\"col-md-6\">
                        <br /><label>Transaction Id</label> : ". $query['transaction_id'] ."
                        <br /><label>Payment Name</label> : ".strtoupper( $processor_name)."
                        <br /><label>Method</label> : ". $payment['method'] ."                     
                        <br /><label>Amount</label> : ". $payment['amount'] ."
                        <br /><label>Date</label> : ".$payment['transaction_date']  ."                       
                        <br /> <label>Currency</label> : ".$payment['currency']."
                     </div>
                    <div class=\"col-md-6\">                     
                        <br /><label>Email</label> : ". $payment['email']."
    ...
    public function getLedgerDebitTransaction($id)
    {
       
        $query = $this->db->query("SELECT * FROM  " . DB_PREFIX . "apsinno_order_transaction_ledger where transaction_id = '" . $id . "' AND transaction_type = 'debit' ");
    ...
    public function updateLedgerDebitTransaction($data)
    {

        if ($data) {
            foreach ($data as $ledger) {

                if( !isset($ledger['ledger_id'] ) || !$ledger['ledger_id'] ){

                    if(isset($ledger['description'] ) && $ledger['description']  && isset($ledger['amount'] ) && $ledger['amount'] ){
                        $query = $this->db->query("insert into " . DB_PREFIX . "apsinno_order_transaction_ledger set transaction_id = '". $ledger['transaction_id']."', transaction_type = 'debit', amount='". $ledger['amount'] ."', description= '". $ledger['description']."' ");
                    }
                 
                }else{

                    if(isset($ledger['description'] ) && $ledger['description']  && isset($ledger['amount'] ) && $ledger['amount'] ){
                        $query = $this->db->query("update " . DB_PREFIX . "apsinno_order_transaction_ledger set description = '" . $ledger['description'] . "', amount='" . $ledger['amount'] . "' where ledger_id = '" . $ledger['ledger_id'] . "'");
                    }
    ...
    public function deleteLedgerDebitTransaction($id)
    {

        if ($id) {
            $query = $this->db->query("delete from " . DB_PREFIX . "apsinno_order_transaction_ledger where ledger_id='".$id."'");
        }


    }

    public function getLedgerTotalDebitTransaction($id)
    {
        $query = $this->db->query("SELECT sum(amount) as total FROM " . DB_PREFIX . "apsinno_order_transaction_ledger where transaction_id = '" . $id . "' And transaction_type='debit'");
